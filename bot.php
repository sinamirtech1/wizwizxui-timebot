<?php
// === PATCH C4: Fix invalid characters and enforce lowercase ===
if (isset($text)) {
    ...
    if ($__shouldHandleNames) {
        ...
        $__base = strtolower(trim($text)); // all lowercase
        // Replace dash and dot with underscores
        $__base = str_replace(['-', '.'], '_', $__base);

        // Validate the base name
        if (!preg_match('/^[a-z0-9_]{3,32}$/', $__base)) {
...
if($botState['subLinkState'] == "on") $acc_text .= "
با کلیک کردن روی لینک اشتراک و باز کردن آن در مرورگر خود، میتوانید حجم و زمان باقی مانده اشتراکتون رو ببینید💎

ویژگی های دیگر لینک اشتراک :دریافت لینک تکی( در صورتی که نمیتوانید از لینک اشتراک استفاده کنید ) ،آموزش اتصال به سرویس ها در سیستم عامل های مختلف ( ios-اندروید-ویندوز ) به همراه لینک دانلود مستقیم برنامه ها⚡️
🌐 لینک اشتراک : \n$subLink
...
if((preg_match('/^discountSelectPlan(\d+)_(\d+)_(\d+)/',$userInfo['step'],$match) || 
    ...
    if( ((isset($respd['price']) && (int)$respd['price'] == 0)) || ($from_id == $admin)){
...
if($data == 'reciveApplications') {
    ...
    $keyboard[] = ['text'=>$buttonValues['back_to_main'],'callback_data'=>"mainMenu"];
    $keyboard = array_chunk($keyboard,1); 
    editText($message_id, "فیلم‌ آموزش اتصال به برنامه جدید Karing ⚡️👇🏻
‏https://t.me/sinamirtech/126 🎥

اگه براتون نصب نشد به پشتیبانی پیام بدید واستون حل کنیم😎
@sinamirtechSupport 👨🏻‍💻پشتیبانی

لینک دانلود مستقیم برنامه‌ها در آخر صفحه👇", json_encode(['inline_keyboard'=>$keyboard]));
}
